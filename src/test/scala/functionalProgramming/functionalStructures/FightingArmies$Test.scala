...
/**
  ...
  */
class FightingArmies$Test extends FunSuite {
  ...
  test("solution 10") {
    ...
    val totalTime = TimeRecd()
    val out = time(totalTime) {
    ...
    println(
      s"""totalTime      :${totalTime() / x}
          |recruitTime   : ${recruitTime() / x}
          |diedTime      : ${diedTime() / x}
          |updateArmyTime: ${updateArmyTime() / x}
          |mergeTime     : ${mergeTime() / x}
          |findArmyTime  : ${findArmyTime() / x}
          |dropTime      : ${dropTime() / x}
          |reverseTime   : ${reverseTime() / x}
          |
      """.stripMargin)
